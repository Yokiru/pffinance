  ...
  return (
    <nav className="fixed bottom-4 left-1/2 -translate-x-1/2 w-auto z-50 flex items-center gap-3">
        ...
        {navItems.map(item => {
          ...
          return (
            <button
              ...
              className={`flex items-center justify-center gap-2 py-2 rounded-full will-change-[width,padding,background-color] transform-gpu ${isActive ? 'bg-white text-black px-6 transition-[width,padding,background-color] duration-200 ease-out' : 'text-gray-400 hover:text-white w-12 transition-colors duration-150'}`}
      ...
      <button
        ...
        className={`w-14 h-14 rounded-full flex items-center justify-center shadow-xl transition-all transform ${isFabActive
